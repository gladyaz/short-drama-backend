...
export interface MigrationRunOptions {
  ...
  limit?: number;
  /**
   * Restricts the run to EXACTLY these `Video.id` values.
   *
   * `limit` alone cannot express "these specific episodes": rows are selected
   * in curated feed order (`sortOrder`), so a bounded run always takes a
   * PREFIX of that order. A controlled wave over any series other than the
   * first one in the feed is therefore impossible to request safely — asking
   * for five rows of the third series would drag in the twenty ahead of it.
   *
   * With this set, a row is a candidate only if its id appears here, so the
   * blast radius is exactly the list an operator typed and nothing adjacent
   * to it. Ordering and `limit` still apply on top; an id that is not
   * eligible (already linked, or no `storageKey`) is simply not returned,
   * and `run` reports it as missing rather than silently proceeding.
   */
  onlyVideoIds?: readonly string[];
}
...
export class R2MediaMigrationService {
  ...
  async run(options: MigrationRunOptions): Promise<MigrationReport> {
    ...
    const onlyVideoIds = options.onlyVideoIds;
    ...
    // An explicitly-named id that produced no row is ALWAYS surfaced. Silently
    // migrating four of the five episodes an operator asked for is exactly the
    // failure a targeted run exists to prevent — the id may be misspelled, or
    // already linked, and those need different responses.
    if (onlyVideoIds) {
      const returned = new Set(rows.map((row) => row.id));
      const missing = onlyVideoIds.filter((id) => !returned.has(id));

      if (missing.length > 0) {
        warnings.push(
          `--only named ${onlyVideoIds.length} id(s) but ${missing.length} matched no ELIGIBLE row ` +
            `(unknown id, empty storageKey, or already linked): ${missing.join(', ')}`,
        );
      }
    }
