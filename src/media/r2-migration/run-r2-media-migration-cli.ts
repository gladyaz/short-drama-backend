...
export function parseMigrationArgs(argv: readonly string[]): {
  ...
  for (const arg of args) {
    if (!known.has(arg) && !arg.startsWith('--limit=')) {
      throw new Error(
        `Unknown argument: ${arg}. Valid: --upload | --verify | --link | --check-remote | --limit=N`,
      );
    }
  }
  ...
  return {
    options: { mode, checkRemote: args.includes('--check-remote'), limit },
  };
}
